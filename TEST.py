...
class DQN(object):
    ...
    def update(self):
        ...
        action_q_value = tf.reduce_sum(tf.multiply(self.q_values, action_input))
        target_q_value = tf.placeholder(tf.float32, [None])
        
        q_value_error = tf.reduce_mean(tf.squared_difference(target_q_value, action_q_value))
        
        update_op = tf.train.GradientDescentOptimizer(learning_rate=0.001).minimize(q_value_error)
        #update_op = tf.train.RMSPropOptimizer(0.001).minimize(q_value_error)
        return action_input, action_q_value, target_q_value, update_op
    ...
    def train(self):
        ...
        while not done:
            ...
            #self.q_values[obs, action] += alpha * (reward + np.max(self.q_values[next_obs]) - self.q_values[obs,action])
            #G += reward
            
            nextQValue = self.sess.run(self.q_values, feed_dict={self.observation_input: next_obs.reshape(1,8)})
            target = reward if done else reward + self.gamma * np.max(nextQValue)
            
            #print(nextQValue)
            action_input, action_q_value, target_q_value, update_op = self.update()
            #print(target)
            actionArray = np.array([0,0,0,0])
            actionArray[action] = 1
            targetArray = np.array([target])
            #print(action)
            self.sess.run(update_op, feed_dict={self.observation_input: obs.reshape(1, 8), action_input: np.array([1,1,1,1]).reshape(1,4), target_q_value: np.array([target])})
            #self.sess.run(update_op, feed_dict={self.observation_input: obs.reshape(1,8), action_input: env.action_space, target_q_value: target})
            #self.sess.run(update_op, feed_dict={self.observation_input: obs.reshape(1,8), action_input: nextQValue, target_q_value: targetArray})
            #self.sess.run(update_op, feed_dict={self.observation_input: obs.reshape(1,8), action_input: actionArray.reshape(1,4), target_q_value: targetArray})
            #print(targetArray)
            #self.replay_memory.push(obs, action, next_obs, reward, 1.0 - done)
            obs = next_obs
            self.num_steps += 1
        self.num_episodes += 1
